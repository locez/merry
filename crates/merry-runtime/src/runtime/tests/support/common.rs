use crate::{
    PermissionReviewMode, RuntimeError, RuntimeModelRole, RuntimeTrustLevel,
...
use merry_core::{
    ArtifactId, ErrorInfo, PendingToolCall, RuntimeJournalEvent, RuntimeJournalPayload, SessionId,
    ToolCallArguments, ToolCallId, ToolName,
};
use merry_llm::{
...
pub(in crate::runtime::tests) fn trace_output_buffer() -> &'static Arc<StdMutex<Vec<u8>>> {
    #[derive(Clone)]
...
pub(in crate::runtime::tests) async fn capture_traces_for<F, R>(
    trace_marker: &str,
    future: F,
) -> (R, String)
where
...
pub(in crate::runtime::tests) fn model_configs_with_primary(
...
pub(in crate::runtime::tests) fn runtime_session_and_plan_controller(
...
pub(in crate::runtime::tests) fn artifact_id(value: &str) -> ArtifactId {
    ArtifactId::new(value).expect("valid artifact id")
}

pub(in crate::runtime::tests) fn session_id(value: &str) -> SessionId {
    SessionId::new(value).expect("valid session id")
}

pub(in crate::runtime::tests) fn model_name() -> ModelName {
    ModelName::new("fake/model").expect("valid model name")
}

pub(in crate::runtime::tests) fn named_model(value: &str) -> ModelName {
    ModelName::new(value).expect("valid model name")
}

pub(in crate::runtime::tests) fn accepted_local_workspace_process_admission()
-> AcceptedLocalWorkspaceProcessAdmission {
    AcceptedLocalWorkspaceProcessAdmission::accept_local_workspace()
}

pub(in crate::runtime::tests) fn completed_event() -> ModelEvent {
...
pub(in crate::runtime::tests) fn completed_event_with(
...
pub(in crate::runtime::tests) fn permission_review_completed_event(
    decision: &str,
    rationale: &str,
) -> ModelEvent {
    let output = format!(
...
pub(in crate::runtime::tests) fn event_kind_names(
...
pub(in crate::runtime::tests) fn failed_code(events: &[RuntimeJournalEvent]) -> Option<&str> {
...
pub(in crate::runtime::tests) async fn collect_step(
    runtime: &Runtime,
    text: &str,
...
pub(in crate::runtime::tests) trait RuntimeSessionStateTestExt {
...
pub(in crate::runtime::tests) fn runtime_test_turn_diagnostic(error: RuntimeError) -> ErrorInfo {
    ErrorInfo::new("test_model_turn", &error.to_string())
        .expect("test model turn diagnostic should be valid")
}

pub(in crate::runtime::tests) fn pending_tool_call(id: &str) -> PendingToolCall {
    PendingToolCall::new(
...
pub(in crate::runtime::tests) fn model_tool_call(id: &str) -> ModelToolCall {
    ModelToolCall::new(
