use crate::{
    ...
    runtime::{
        ...
        tests::support::{
            ...
            model_provider::{NotifyOnDrop, RecordingModelProvider},
            runtime_factories::runtime_with_provider_and_memory_source,
        },
    },
};
use merry_core::{
    ArtifactKind, ArtifactRef, EvidenceLocator, EvidenceRef, PendingToolCall, ToolCallArguments,
...
pub(in crate::runtime::tests) fn activated_memory(
    id: &str,
    text: &str,
...
pub(in crate::runtime::tests) fn memory_item(
    id: &str,
    text: &str,
    evidence_artifact: &str,
    triggers: &[&str],
...
pub(in crate::runtime::tests) fn activated_memory_with_unreadable_evidence(
    id: &str,
) -> ActivatedMemory {
    activated_memory(id, "Unreadable evidence memory.", &format!("{id}-missing"))
}

pub(in crate::runtime::tests) fn record_memory_artifact(
    runtime: &Runtime,
...
pub(in crate::runtime::tests) fn record_prior_failed_tool_result(runtime: &Runtime, content: &str) {
    let mut session = runtime
        .inner
        .session
        .try_lock()
...
pub(in crate::runtime::tests) fn record_memory_item(runtime: &Runtime, item: MemoryItem) {
    let mut session = runtime
        .inner
        .session
        .try_lock()
...
pub(in crate::runtime::tests) fn runtime_with_provider_and_single_memory(
    session: &str,
...
pub(in crate::runtime::tests) async fn compiled_context_snapshot(runtime: &Runtime) -> String {
...
pub(in crate::runtime::tests) async fn assert_activated_memory_projection_retained(
    runtime: &Runtime,
    memory_id: &str,
    memory_text: &str,
) {
...
#[derive(Debug)]
pub(in crate::runtime::tests) enum ScriptedMemoryActivationResponse {
...
impl ScriptedMemoryActivationResponse {
    pub(in crate::runtime::tests) async fn into_result(
        self,
...
#[derive(Debug, Clone)]
pub(in crate::runtime::tests) struct ScriptedMemoryActivationSource {
    pub(in crate::runtime::tests) responses: Arc<StdMutex<Vec<ScriptedMemoryActivationResponse>>>,
    pub(in crate::runtime::tests) calls: Arc<AtomicUsize>,
    pub(in crate::runtime::tests) observed_queries: Arc<StdMutex<Vec<String>>>,
}

impl ScriptedMemoryActivationSource {
    pub(in crate::runtime::tests) fn new(responses: Vec<Vec<ActivatedMemory>>) -> Self {
        Self::with_script(
            responses
                .into_iter()
    ...
    pub(in crate::runtime::tests) fn with_script(
        responses: Vec<ScriptedMemoryActivationResponse>,
    ) -> Self {
        Self {
...
pub(in crate::runtime::tests) fn pending_memory_activation_source() -> (
