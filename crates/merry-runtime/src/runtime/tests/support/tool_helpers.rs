use crate::{
    ...
    runtime::{
        DIAGNOSTIC_TOOL_ACTION_POLICY_DENIED, Runtime, RuntimeBuilder,
        TOOL_ACTION_POLICY_DENIED_MESSAGE,
        tests::support::common::{RuntimeSessionStateTestExt, session_id},
    },
    tool::{RegisteredTool, ToolActionKind, ToolExecutor},
};
use merry_core::{
    PendingToolCall, RuntimeJournalEvent, RuntimeJournalPayload, ToolCallArguments, ToolCallId,
...
pub(in crate::runtime::tests) fn registered_tool_spec() -> ToolSpec {
    let schema =
...
pub(in crate::runtime::tests) fn policy_tool_spec(name: &str) -> ToolSpec {
    let schema =
        Schema::try_from(json!({ "type": "object" })).expect("test schema should be a JSON schema");
    ToolSpec::new(
...
pub(in crate::runtime::tests) fn required_query_tool_spec(name: &str) -> ToolSpec {
...
pub(in crate::runtime::tests) fn policy_pending_tool_call(id: &str, name: &str) -> PendingToolCall {
    PendingToolCall::new(
...
pub(in crate::runtime::tests) fn permission_pending_tool_call(
    id: &str,
    reason: &str,
    command: &str,
) -> PendingToolCall {
...
pub(in crate::runtime::tests) fn path_permission_pending_tool_call(
    id: &str,
    path: &str,
    access: &str,
) -> PendingToolCall {
...
pub(in crate::runtime::tests) fn invalid_permission_pending_tool_call(id: &str) -> PendingToolCall {
    PendingToolCall::new(
        ToolCallId::new(id).expect("valid tool call id"),
        ToolName::new("request_permissions").expect("valid tool name"),
        ToolCallArguments::try_from(serde_json::json!({
...
pub(in crate::runtime::tests) fn resolved_tool_result(
    events: &[RuntimeJournalEvent],
) -> &merry_core::ToolCallResult {
    events
        .iter()
...
pub(in crate::runtime::tests) async fn register_policy_pending_registered_tool(
    session: &str,
    tool_name: &str,
    call_id: &str,
    tool: RegisteredTool,
...
pub(in crate::runtime::tests) async fn register_policy_pending_registered_tool_with_builder(
    session: &str,
    tool_name: &str,
    call_id: &str,
    tool: RegisteredTool,
...
pub(in crate::runtime::tests) async fn register_permission_pending_tool_with_builder(
...
pub(in crate::runtime::tests) async fn denied_action_content(
    runtime: &Runtime,
    events: &[RuntimeJournalEvent],
) -> serde_json::Value {
...
pub(in crate::runtime::tests) fn lifecycle_kinds(
...
pub(in crate::runtime::tests) fn assert_lifecycle_order(
...
pub(in crate::runtime::tests) fn assert_sanitized_policy_denial_content(
...
pub(in crate::runtime::tests) fn event_kind_names_for_tool_execution(
    events: &[RuntimeJournalEvent],
) -> Vec<&'static str> {
    events
        .iter()
