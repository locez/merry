...
fn ready_set_is_deterministic_and_dependency_completion_releases_next_leaf() {
    ...
    let worker = actor("worker-first");
    let started = plan
        .start_attempt(&ids["first"], worker.clone(), 1_000)
...
fn multiple_progress_reports_stay_inside_one_attempt_and_apply_directive_explicitly() {
    ...
    let worker = actor("worker-progress");
    let started = plan
        .start_attempt(&ids["work"], worker.clone(), 1_000)
...
#[test]
fn queued_directive_is_delivered_once_at_an_explicit_worker_boundary() {
    let (mut plan, ids) = executing_plan(root(vec![leaf("work", "Steered work")]));
    let worker = actor("worker-delivery");
    let started = plan
        .start_attempt(&ids["work"], worker.clone(), 1_000)
        .expect("attempt starts");
    let queued = plan
    ...
    let delivered = plan
        .deliver_queued_directives(&worker, &started.lease.lease_id, 2_000)
    ...
    let repeated = plan
        .deliver_queued_directives(&worker, &started.lease.lease_id, 2_500)
...
fn expired_lease_is_interrupted_and_requeued_without_replaying_directives() {
    ...
    let worker = actor("worker-expired");
    let started = plan
        .start_attempt(&ids["work"], worker, 1_000)
        .expect("attempt starts");
    let directive = plan
        .issue_directive(
            ControlPlanAttemptInput {
                ...
                expected_node_revision: started.lease.node_revision,
                kind: PlanDirectiveKind::RequestStatus,
                reason: "Check worker liveness".to_owned(),
                instruction: None,
                constraints: None,
    ...
    let replacement = plan
        .start_attempt(&ids["work"], actor("worker-retry"), 40_000)
...
fn elapsed_time_requests_review_at_a_safe_boundary_without_cancelling_the_attempt() {
    ...
    let worker = actor("worker-review");
    let started = plan
        .start_attempt(&ids["work"], worker.clone(), 1_000)
        .expect("attempt starts");
    let window = plan
    ...
    let early = plan
        .review_progress_at_boundary(&worker, &started.lease.lease_id, 1_000 + window - 1)
        .expect("early review check succeeds");
    assert!(early.updated_progress.is_none());

    let due = plan
        .review_progress_at_boundary(&worker, &started.lease.lease_id, 1_000 + window)
...
fn durable_progress_moves_the_review_window_forward() {
    ...
    let worker = actor("worker-durable-progress");
    let started = plan
        .start_attempt(&ids["work"], worker.clone(), 1_000)
        .expect("attempt starts");
    let window = plan
    ...
    let early = plan
        .review_progress_at_boundary(&worker, &started.lease.lease_id, 500_000 + window - 1)
        .expect("review check succeeds");
    assert!(early.updated_progress.is_none());
    let due = plan
        .review_progress_at_boundary(&worker, &started.lease.lease_id, 500_000 + window)
...
fn terminal_attempt_report_is_exactly_once() {
    ...
    let worker = actor("worker-once");
    let started = plan
        .start_attempt(&ids["work"], worker.clone(), 10)
        .expect("attempt starts");
    ...
    plan.report_attempt(&worker, report.clone(), 20)
        .expect("first report succeeds");
    let before = plan.snapshot().clone();
    let error = plan
        .report_attempt(&worker, report, 30)
        .expect_err("duplicate report rejects");

    assert!(matches!(error, crate::plan::PlanError::LeaseNotLive { .. }));
    assert_eq!(plan.snapshot(), &before);
}

#[test]
fn worker_decomposition_adds_only_direct_children_and_releases_them() {
    let (mut plan, ids) = executing_plan(root(vec![leaf("work", "Decomposable work")]));
    let worker = actor("worker-decompose");
    let started = plan
        .start_attempt(&ids["work"], worker.clone(), 10)
        .expect("attempt starts");
    let report = plan
...
#[test]
fn nested_worker_decomposition_rejects_without_resolving_attempt() {
    let (mut plan, ids) = executing_plan(root(vec![leaf("work", "Decomposable work")]));
    let worker = actor("worker-invalid-decompose");
    let started = plan
        .start_attempt(&ids["work"], worker.clone(), 10)
        .expect("attempt starts");
...
fn transient_retry_uses_a_new_attempt_and_blocks_at_policy_limit() {
    ...
    let first_worker = actor("worker-retry-one");
    let first = plan
    ...
    let second_worker = actor("worker-retry-two");
    let second = plan
        .start_attempt(&ids["work"], second_worker.clone(), 30)
