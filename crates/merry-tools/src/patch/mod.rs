...
#[cfg(test)]
pub(crate) use plan::apply_patch_blocking;
pub(crate) use plan::{apply_patch_blocking_checked, propose_apply_patch_blocking_checked};
#[cfg(test)]
pub(crate) use types::stable_content_fingerprint;

#[derive(Debug)]
pub(crate) struct ApplyPatchExecutor {
...
impl ToolExecutor for ApplyPatchExecutor {
    fn propose<'a>(
        ...
        Box::pin(async move {
            ...
            let handle = tokio::task::spawn_blocking(move || {
                ...
                propose_apply_patch_blocking_checked(&state, args, &call, &is_cancelled)
            });
    ...
    fn execute<'a>(
        ...
        Box::pin(async move {
            ...
            let token = context.cancellation_token().clone();
            let worker_token = token.clone();
            let approved_proposal = context.approved_apply_patch().cloned();
            let handle = tokio::task::spawn_blocking(move || {
                let is_cancelled = || worker_token.is_cancelled();
                apply_patch_blocking_checked(
                    &state,
                    args,
