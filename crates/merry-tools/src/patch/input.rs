...
#[merry_tools_macros::tool(
    crate = "crate",
    name = "apply_patch",
    description = "Apply one constrained patch of UTF-8 files inside the workspace. Send exactly one envelope: `*** Begin Patch` ... `*** End Patch`. Inside it, use the section that matches the intent: `*** Add File: <path>` with every content line prefixed `+` (creates missing parent directories, never overwrites an existing path); `*** Update File: <path>` with hunks started by `@@` and every line prefixed with one space for context, `+` for an added line, or `-` for a removed line; `*** Delete File: <path>` to remove an existing file and leave its parent directory in place. Name each file in at most one Add or Delete section, and repeat `*** Update File:` sections for one file only when that is easier than one section with several hunks, because repeated update sections merge into a single change. Hunks apply in order and each one must match the current file bytes exactly once, so keep context minimal but unique and re-read the file when it may have changed; a hunk with only context lines is dropped unless the envelope has no `+` or `-` line at all, which is rejected. The whole envelope is planned before anything is written, so one unmatched path or hunk writes nothing. A section path is relative to the workspace root or absolute inside it, and a path outside the workspace is denied, so note the reported workspace-relative path when you need to refer to the file again. The patch payload and resulting writes are bounded by configured limits."
)]
#[derive(Debug, Deserialize, JsonSchema)]
#[serde(deny_unknown_fields)]
pub(crate) struct ApplyPatchInput {
    #[schemars(
        description = "Patch envelope containing one or more Add, Update, or Delete file sections inside the workspace.",
        length(min = 1)
    )]
