...
fn config_rejects_each_zero_limit() {
    ...
    for invalid_name in [
        ...
        match invalid_name {
            ...
            "max_read_lines" => limits.max_read_lines = 0,
...
#[test]
fn into_registered_tools_exposes_read_text_only() {
    ...
    assert_eq!(names, [READ_TEXT_TOOL]);
    assert!(
        tools
            .iter()
...
fn patch_tool_registration_is_opt_in_and_workspace_write() {
    ...
    assert!(
        read_only_tools
            .iter()
            .all(|tool| tool.spec().name().as_str() != APPLY_PATCH_TOOL)
    );
    ...
    assert!(
        tools
            .iter()
            .filter(|tool| tool.spec().name().as_str() != APPLY_PATCH_TOOL)
            .all(|tool| {
