...
pub(super) use merry_tools::{
    APPLY_PATCH_TOOL, READ_TEXT_TOOL, WorkspaceTools, WorkspaceToolsConfig,
};
...
impl TempWorkspace {
    pub(super) fn new(name: &str) -> Self {
        ...
        let root = env::temp_dir().join(format!(
            "merry-tools-runtime-{name}-{}-{unique}",
            std::process::id()
        ));
...
pub(super) fn pending_read_text_call(path: &str) -> ModelEvent {
    let mut arguments = Map::new();
    arguments.insert("path".to_owned(), Value::String(path.to_owned()));
    pending_workspace_call("workspace-read-call", READ_TEXT_TOOL, arguments)
}
...
pub(super) fn runtime_with_opt_in_apply_patch_tools(
    root: &Path,
    model_event: ModelEvent,
) -> Runtime {
    let tools = WorkspaceTools::new(WorkspaceToolsConfig::new(vec![root.to_path_buf()]))
...
pub(super) fn runtime_with_opt_in_apply_patch_tools_and_provider(
    root: &Path,
