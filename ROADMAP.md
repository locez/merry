...
T5 -> T6
T5 + T6 -> T7 -> T8
T9 is an independent Harbor manual/nightly path
T5 + T7 + T8 -> T10
T6 + T9 + T10 -> T11
```

T1 remains a completed protocol prototype, but `merry-eval` is deferred and a
candidate for removal. T2 and T4 are closed as standalone delivery milestones:
deterministic Rust tests cover runtime contracts, while Harbor owns external
benchmark execution and model-quality comparison. No current delivery path may
extend or depend on `merry-eval`.
